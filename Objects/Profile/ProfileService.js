module.exports = class ProfileService {
    ...
    async GetUsersFriends(people, settings) {
        if (!people || !Array.isArray(people))
            throw new Error('People must be an array of userids')
        if (!settings || !Array.isArray(settings))
            throw new Error('Settings must be an array of settings')
    ...
    async _GetUserProfilesForSocialGroup(person, settings, socialGroup) {
        if (!settings || !Array.isArray(settings))
            throw new Error('Settings must be an array of settings')
        return await this.client.base.profile.api.users[
            person
        ].profile.settings.people[socialGroup].get({
            query: {
    ...
    async GetUserFriends(userId, settings, socialGroup = 'people') {
        return await this._GetUserProfilesForSocialGroup(
            'xuid(' + userId + ')',
            settings,
            socialGroup
        )
