...
function parseResponse(res) {
  if (res.headers.get('content-type') && res.headers.get('content-type').startsWith('application/json')) return res.json();
  return res.json();
}
...
class RequestHandler {
  ...
  async execute(request) {
    ...
    if ((this.manager.client.listenerCount && this.manager.client.listenerCount(API_REQUEST))) {
      /**
    ...
    if (this.manager.client.listenerCount && this.manager.client.listenerCount(API_RESPONSE)) {
      /**
    ...
    Sublimit: ${sublimitTimeout ? `${sublimitTimeout}ms` : 'None'}`,
        ...
        if (sublimitTimeout) {
          await delayFor(sublimitTimeout);
        }
        return this.execute(request);
      }
