...
class Util extends null {
     ...
     */
    static async fetchRecommendedShards(
        token,
        ...
        const response = await fetch(
            `${defaults.http.api}/v${defaults.http.version}${Endpoints.botGateway}`,
            {
                method: 'GET',
                headers: {
        ...
        return (
            Math.ceil((shards * (1_000 / guildsPerShard)) / multipleOf) *
            multipleOf
        )
    }
    ...
    static basename(path, ext) {
        ...
        return ext && res.ext.startsWith(ext)
            ? res.name
            : res.base.split('?')[0]
    }
        ...
        str = str
            .replace(/<@!?[0-9]+>/g, (input) => {
                ...
                if (channel.type === 'DM') {
                    ...
                    return user
                        ? Util.removeMentions(`@${user.username}`)
                        : input
                }
