...
class LimitedCollection extends Collection {
    ...
    set(key, value) {
        ...
        if (this.size >= this.maxSize && !this.has(key)) {
            for (const [k, v] of this.entries()) {
                const keep =
                    this.keepOverLimit && this.keepOverLimit(v, k, this) == null
                        ? false
                        : this.keepOverLimit && this.keepOverLimit(v, k, this)
                if (!keep) {
                    this.delete(k)
                    break
                }
    ...
    static filterByLifetime({
        lifetime = 14400,
        getComparisonTimestamp = (e) => e && e.createdTimestamp,
        ...
        return () => {
            ...
            return (entry, key, coll) => {
                ...
                const comparisonTimestamp = getComparisonTimestamp(
                    entry,
                    key,
                    coll
                )
                if (
                    !comparisonTimestamp ||
                    typeof comparisonTimestamp !== 'number'
                )
                    return false
